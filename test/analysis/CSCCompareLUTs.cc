      ...
      for(unsigned int address = 0; address < 1<<CSCBitWidths::kGlobalPhiAddressWidth; ++address)
	{
	  ...
	  if(LUT_1 != LUT_2)
	      ...
	      if(legalAddress)
		{
		  ...
		  mismatchGlobalPhiMBAddress_cscId->Fill((address & 0x7800) >> 15);
		}
  ...
  if(doGlobalEta)
      ...
      for(unsigned int address = 0; address < 1<<CSCBitWidths::kGlobalEtaAddressWidth; ++address)
	{
	  ...
	  double float_eta = getGlobalEtaValue(_endcap, _sector, _subsector, station,
					       ((address & 0x7800) >> 15), ((address & 0x7f00) >> 8), ((address & 0xc0) >> 6));
	  if ((float_eta < CSCTFConstants::minEta) || (float_eta >= CSCTFConstants::maxEta) || // eta out of range
	      ((address & 0x7800) >> 15) < CSCTriggerNumbering::minTriggerCscId()           || // CSC ID out of range
	      ((address & 0x7800) >> 15) > CSCTriggerNumbering::maxTriggerCscId() )            // CSC ID out of range
	    {
	      legalAddress=false;
	    }
	  ...
	  compareGlobalEta_phiBend->Fill(((LUT_1 & 0xf80)>>7), ((LUT_2 & 0xf80)>>7));
	  ...
	  if(LUT_1 != LUT_2)
	      ...
	      if(legalAddress)
		{
		  ...
		  mismatchGlobalEtaAddress_phiLocal->Fill((address & 0xc0) >> 6);
		  mismatchGlobalEtaAddress_wireGroup->Fill((address & 0x7f00) >> 8);
		  mismatchGlobalEtaAddress_cscId->Fill((address & 0x7800) >> 15);
		}
	      ...
	      differenceGlobalEta_phiBend->Fill(((LUT_1 & 0xf80)>>7) - ((LUT_2 & 0xf80)>>7));
	    }
	  ...
	  if((LUT_1 & 0xf80) != (LUT_2 & 0xf80)) // if phi bend word does not match
	    {
	      compareGlobalEtaOffDiagonal_phiBend->Fill(((LUT_1 & 0xf80)>>10), ((LUT_2 & 0xf80)>>10));
	    }
